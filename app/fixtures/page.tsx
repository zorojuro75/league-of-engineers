...
const Page = (props: Props) => {
  ...
  return (
    ...
    <div className="w-max-7xl shadow-xl bg-white mx-auto grid md:grid-cols-2 grid-cols-1 place-content-between gap-x-20 gap-y-5 p-10 rounded-lg my-10 text-sm md:text-lg">
      <div className="md:col-span-2 text-center text-3xl font-bold">Semi Final</div>
    {fixturesSF.map((match) => (
              ...
              >
                ...
                {
                match.played!=true?<div className="border-s border-black border-opacity-40 md:ps-5 ps-1">
                <div>{match.matchDate}</div>
                <div>{match.matchTime}</div>
                <div>{match.matchDay}</div>
              </div>:
              <div className="border-s border-black border-opacity-40 md:ps-5 ps-1">FT</div>
                }
    ...
    {fixturesQF.length!=0?
    <div className="w-max-7xl shadow-xl bg-white mx-auto grid md:grid-cols-2 grid-cols-1 place-content-between gap-x-20 gap-y-5 p-10 rounded-lg my-10 text-sm md:text-lg">
      <div className="md:col-span-2 text-center text-3xl font-bold">Quarter Final</div>
    {fixturesQF.map((match) => (
      ...
      <div className="w-max-7xl shadow-xl bg-white mx-auto grid md:grid-cols-2 grid-cols-1 place-content-between gap-x-20 gap-y-5 p-10 rounded-lg my-10 text-sm md:text-lg">
        <div className="md:col-span-2 text-center text-3xl font-bold">Group Stage</div>
        {fixtures
          ? fixtures.map((match) => (
              <div
                key={match.matchID}
              ...
              <div className="border-s border-black border-opacity-40 md:ps-5 ps-1">Full Time</div>
                }
              </div>
            ))
          : null}
